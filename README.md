...
- **Testing**: Unit test templates for Jest (database queries, Redux slices, WebSocket logic)
...
7. **Jest Configuration**: Set up Jest and run the unit test suite
...
Unit test templates are provided in `__tests__` directories:

- `src/server/__tests__/database.test.ts` - Database query tests
- `src/store/__tests__/connectionSlice.test.ts` - Connection state machine tests
- `src/store/__tests__/slices.test.ts` - Redux slices tests
- `src/hooks/__tests__/useWebSocket.test.ts` - WebSocket logic tests
